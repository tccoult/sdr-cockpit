...
def _map_status(proto_status: BitTestState) -> BitStatus:
    """
...
class BitProtoMapper:
    """
    Maps proto dataclasses (TestResults + ReportRollupMsg) to BitResult.
    ...
    def map_to_bit_result(
        ...
        function_node_map = self._build_function_node_map(rollup.functions)

        # Convert tests
        tests = [self._map_test(test, function_node_map) for test in test_results.tests]
        tests_by_name = {t.name: t for t in tests}

        # Build function tree using proto structure but with test status from TestResults
        function_tree = self._build_function_tree(rollup.functions, tests_by_name)
    ...
    def _map_test(
        ...
        return BitTest(
            id=test_name,
            name=test_name,
            status=_map_status(proto_test.status),
            description="",  # Not provided by proto, to be added later
        ...
        def convert_node(node: FunctionStatusTree) -> BitTreeNode:
            ...
            if is_leaf:
                ...
                if test:
                    ...
                    status = _map_status(node.status)
                    tests_list = []
            else:
