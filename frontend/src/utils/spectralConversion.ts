...
/**
 * Decode a batch of FFT frames, respecting delta encoding flags.
 *
 * Each frame may be absolute or delta-encoded based on its is_delta flag.
 * Delta frames contain differences from the previous frame.
 *
 * @param batchFrames Array of frames with bins (Uint8Array) and is_delta flag
 * @returns Array of dB bin values for each frame
 */
export function decodeDeltaBatch(
  batchFrames: Array<{ bins: Uint8Array; isDelta?: boolean }>
): Float32Array[] {
  if (batchFrames.length === 0) return [];

  const result: Float32Array[] = [];
  const numBins = batchFrames[0].bins.byteLength / 2;
  let previousInt16: Int16Array | null = null;

  for (let frameIdx = 0; frameIdx < batchFrames.length; frameIdx++) {
    ...
    if (frame.isDelta && previousInt16 !== null) {
      // Delta frame: reconstruct from previous + delta
      ...
      // Absolute frame: decode directly
      if (previousInt16 === null) {
        previousInt16 = new Int16Array(numBins);
      }
